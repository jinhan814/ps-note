...
auto sol = [](int n, int q, auto v, auto qs) {
	...
	segtree t2(n);
	for (int i = 0; i < n; i++) {
		int p = lower_bound(c.begin(), c.end(), pair(v[i], i)) - c.begin() + 1;
		t1.update(p, 1);
		t2.update(i, node(v[i]));
	}
	auto update = [&](int i, int x) {
		int p1 = lower_bound(c.begin(), c.end(), pair(v[i], i)) - c.begin() + 1;
		t1.update(p1, -1);
		v[i] = x;
		int p2 = lower_bound(c.begin(), c.end(), pair(v[i], i)) - c.begin() + 1;
		t1.update(p2, 1);
		t2.update(i, node(v[i]));
	};
	auto query = [&](int l, int r) {
		int val = t2.query(l, r).val;
		int p1 = lower_bound(c.begin(), c.end(), pair(val, l)) - c.begin() + 1;
		int p2 = upper_bound(c.begin(), c.end(), pair(val, r)) - c.begin();
		int cnt = t1.query(p1, p2);
